...
@pytest.mark.parametrize(
    "filename,partial_index",
    [
        ("tests/classes.py", {25: 28, 32: 35, 36: 37, 46: 48}),
    ],
)
def test_line_index_builder_for_class(filename: str, partial_index: dict[int, int]):
    """`partial_index` contains all line numbers that should be different (one-based indexing)"""
    with open(os.path.join(os.path.dirname(__file__), filename)) as f:
        source = f.read()

    index = {}
    for i in range(len(source.splitlines())):
        node = FunctionFinder.find_function(source, LineNumber[0](i))
        if node:
            index |= LineIndexBuilder.build_index(node)
